...
export async function waitForDeskBrowser(deskId, timeoutMs = 24000, fetchImpl = fetch, retryMs = 400) {
  const t0 = Date.now();
  let last = "工作区还没准备好";
  while (Date.now() - t0 < timeoutMs) {
    try {
      return await deskBrowserWs(deskId, fetchImpl);
    } catch (e) {
...
export async function createTargetViaHttp(deskId, startUrl = CHATGPT_START, fetchImpl = fetch) {
  const urls = [deskJsonNewUrl(deskId, startUrl), `${deskCdpBase(deskId)}/json/new?${startUrl}`];
  let last = "无法创建分屏席位";
...
async function parkBestEffort(deskId, targetId, { fetchImpl, connect }) {
  try {
    const browserWs = await deskBrowserWs(deskId, fetchImpl);
    const cdp = connect(browserWs);
    await cdp.ready;
...
async function createTargetViaBrowser(deskId, { fetchImpl, connect, startUrl }) {
  const browserWs = await deskBrowserWs(deskId, fetchImpl);
  const cdp = connect(browserWs, 2000);
  await withDeadline(cdp.ready, 2000, "无法连接页面");
  try {
    let created;
    try {
      ...
      created = await withDeadline(cdp.send("Target.createTarget", { url: startUrl }), 2500, "无法创建分屏席位");
    }
...
export async function createParkedChatGPTTab(
  ...
  while (Date.now() - t0 < timeoutMs) {
    try {
      return await createTargetViaBrowser(deskId, { fetchImpl, connect, startUrl });
    } catch (e) {
      last = e.message || last;
    }
    try {
      const created = await createTargetViaHttp(deskId, startUrl, fetchImpl);
      await parkBestEffort(deskId, created.targetId, { fetchImpl, connect });
      return created;
    } catch (e) {
      last = e.message || last;
    }
