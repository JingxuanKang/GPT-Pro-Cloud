...
/**
 * One browser-level DevTools socket per desk.
...
export async function createParkedChatGPTTab(
  ...
  try {
    ...
    try {
      ...
      const existing = pickUnclaimedChatGPTTarget(pages, { claimedTargetIds, deskId });
      if (existing && reserveTarget(deskId, existing.id)) {
        try {
          await parkTargetWindow(cdp, existing.id);
        } catch {
          /* parking is best-effort */
        }
        return { targetId: existing.id, adopted: true };
      }
    } catch {
      /* no existing target — create a background tab */
    }
    let created;
    try {
      created = await withDeadline(
        cdp.send("Target.createTarget", { url: startUrl, newWindow: false, background: true }),
        4000,
        "无法创建分屏席位",
      );
    } catch {
      created = await withDeadline(cdp.send("Target.createTarget", { url: startUrl }), 4000, "无法创建分屏席位");
    }
...
export async function closeTarget(deskId, targetId, { fetchImpl = fetch, connect = createCdpConnection, pool = deskBrowsers } = {}) {
  if (!targetId) return false;
...
export async function attachSeatTarget(deskId, targetId, { fetchImpl = fetch, connect = createCdpConnection, pool = deskBrowsers } = {}) {
  if (!targetId) throw new Error("没有分屏目标");
  const cdp = await pool.get(deskId, { fetchImpl, connect });
  const attached = await cdp.send("Target.attachToTarget", { targetId, flatten: true });
  const sessionId = attached?.sessionId;
  if (!sessionId) throw new Error("无法连接分屏席位");
  const release = () => cdp.send("Target.detachFromTarget", { sessionId }).catch(() => {});
  return { cdp, sessionId, targetId, release };
}
