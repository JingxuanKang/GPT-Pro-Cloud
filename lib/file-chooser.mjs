 ...
 * Intercept Page.fileChooserOpened, let the user's OS pick, then apply
 * via DOM.setFileInputFiles (paths staged in the desk container).
 */
...
export const FILE_CDP_OFF = "未开多人分屏时无法上传该文件，请粘贴图片，或请管理员开启多人分屏";
...
export function isPasteableUploadImage(mime) {
  return /^image\/(png|jpeg|jpg|webp)$/i.test(String(mime || ""));
}
...
export async function applyDeskUpload({
  ...
  if (!cdpOn) {
    if (!classified.files.every((f) => isPasteableUploadImage(f.mime))) {
      return { ok: false, error: FILE_CDP_OFF, status: 409 };
    }
    if (typeof pasteImages !== "function") return { ok: false, error: FILE_CDP_OFF, status: 409 };
    for (const img of classified.files) {
      const out = await pasteImages(img);
      if (!out?.ok) return { ok: false, error: out?.error || FILE_UPLOAD_FAIL, status: out?.status || 502 };
    }
