...
export const DOCKER_REMOVE_MS = 8_000;
...
export function createDockerClient({ socketPath = "/var/run/docker.sock", templateName = "gpt-pro-cloud-a" } = {}) {
  const request = (method, path, body, { timeoutMs = DOCKER_REQUEST_MS } = {}) =>
    new Promise((resolve, reject) => {
      ...
      req.setTimeout(timeoutMs, () => {
        req.destroy();
        reject(dockerError(method === "DELETE" ? "拆除账号容器超时，请稍后重试" : "Docker 操作超时", 502));
      });
  ...
  return {
    ...
    async remove(name) {
      return request("DELETE", `/containers/${encodeURIComponent(name)}?force=true&v=true`, undefined, {
        timeoutMs: DOCKER_REMOVE_MS,
      });
    },
    async wait(name) {
...
export async function removeDeskContainer(id, client) {
  ...
  try {
    ...
    if (e.status !== 404) {
      throw dockerError(e.message || "无法拆除账号容器", e.status || 502, e);
    }
  }
