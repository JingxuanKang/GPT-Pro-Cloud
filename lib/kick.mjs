...
export function kickLiveSession({ sessions, presence, sockets, seats }, userId) {
  const id = String(userId || "");
  if (!id) return { sessions: 0, sockets: 0, seats: 0, released: [] };
  const droppedSessions = sessions?.deleteByUser?.(id) || 0;
  presence?.leaveAll?.(id);
  const droppedSockets = sockets?.drop?.(id) || 0;
  const released = seats?.releaseByUser?.(id) || [];
  return { sessions: droppedSessions, sockets: droppedSockets, seats: released.length, released };
}
