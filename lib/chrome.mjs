...
export function projectOnboardScript(name, { create = true } = {}) {
  ...
  return `(() => {
    ...
    const projectHref = () => {
      if (/\\/g\\/g-p-[^/?#]+/i.test(location.href)) return location.href;
      const item = findProject();
    ...
    const inProject = () => {
      ...
      return document.title === "ChatGPT - " + want || !!h1 || !!findProject() || /\\/g\\/g-p-[^/?#]+/i.test(location.href);
    };
    ...
    return (async () => {
      ...
      if (existing) {
        ...
        while (Date.now() < until) {
          if (inProject()) return { ok: true, action: "opened", url: projectHref() };
          await sleep(200);
        }
        return { ok: true, action: "opened", url: projectHref() };
      }
