...
/** Tear down a panel-created desk. Seed compose seats (a/b) cannot be removed. */
export async function retireDesk({ users, registry, remove, id }) {
  const deskId = String(id || "").trim();
  if (!deskId) throw fail("账号不存在", 404);
  if (!users.listDeskIds().includes(deskId)) throw fail("账号不存在", 404);
  if (!users.isExtraDesk(deskId)) throw fail("内置账号不能删除", 400);
  await remove(deskId);
  users.removeDesk(deskId);
  registry.remove(deskId);
  return { id: deskId };
}
