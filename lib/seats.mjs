 ...
 * - No ChatGPT cookies yet → VNC so the owner can sign in.
 * - Same member already seated → attach that seat (no second tab).
 * - Cookies exist and someone else is already on the desk → new tab seat.
 * - Cookies exist and the desk is empty → VNC (existing first-occupant path).
 */
export function decideOpenMode({ occupants = [], userId, hasSession, tabCount = 0, cap = DEFAULT_TAB_SEAT_CAP, existing } = {}) {
  if (existing) return { mode: existing.mode, attach: true, seat: existing };
  const others = occupants.filter((o) => o.userId && o.userId !== userId);
  if (!hasSession) return { mode: "vnc", attach: false };
  if (others.length === 0) return { mode: "vnc", attach: false };
  if (tabCount >= cap) throw seatCapError(cap);
  return { mode: "tab", attach: false };
}
  ...
  return {
    ...
    decide(deskId, user, { hasSession } = {}) {
      ...
      return decideOpenMode({
        occupants: seats.map((s) => ({ userId: s.userId })),
        userId: user.id,
        hasSession,
        tabCount,
        cap,
