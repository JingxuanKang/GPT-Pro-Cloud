  ...
  return shell(`<div class="narrow">
    ...
    <section class="panel">
      <div class="panel-head">
        <b>多人分屏</b>
        <em>多人分屏暂未开放。每个账号同时只允许一人进入，走独占桌面。</em>
      </div>
    </section>
    <section class="panel">
      <div class="panel-head">
...
function renderDesk() {
  ...
  return `<div class="stage">
    ...
    <div class="frame">
      ...
      <input type="file" id="os-files" multiple hidden />
...
function bindSeatCast() {
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.type === "file-chooser") {
      onRemoteFileChooser(msg.mode).catch(() => {});
      return;
    }
  ...
  canvas.ondrop = (e) => {
    ...
    const p = point(e);
    sendOsFiles(list, { drop: { x: p.x, y: p.y } }).catch(() => {});
  };
...
function bindDesk() {
  ...
  bindDeskFileDrop();
  if (deskCdpOn(state.deskId) && state.deskMode === "vnc") startChooserPoll();
