...
import {
  ...
  FILE_TOO_BIG,
  FILE_UPLOAD_MAX,
  isPasteableUploadImage,
  safeUploadName,
} from "../lib/file-chooser.mjs";
import { deskUploadDir, packUstar } from "../lib/desk-files.mjs";
import { classifyTabPaste, tabPastePlan } from "../lib/tab-paste.mjs";

describe("local file apply", () => {
  it("rejects oversized and empty uploads with a Chinese error", () => {
    ...
    assert.equal(over.error.includes("12MB"), true);
  ...
  it("applyDeskUpload uses staged paths and wipes them", async () => {
    const wiped = [];
    const sent = [];
    const out = await applyDeskUpload({
      files: [{ name: "brief.docx", mime: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", bytes: Buffer.from("PK") }],
      cdpOn: true,
      targetId: "t-ada",
      pending: { backendNodeId: 7, targetId: "t-ada" },
      attach: async () => ({
        cdp: {
  ...
  it("CDP-off rejects non-image files and still allows png paste", async () => {
    const pdf = await applyDeskUpload({
      files: [{ name: "a.pdf", mime: "application/pdf", bytes: Buffer.from("%PDF") }],
      cdpOn: false,
    });
    assert.equal(pdf.ok, false);
    assert.equal(pdf.error, FILE_CDP_OFF);
    const pasted = [];
    const img = await applyDeskUpload({
      files: [{ name: "x.png", mime: "image/png", bytes: Buffer.from([1, 2, 3]) }],
      cdpOn: false,
      pasteImages: async (file) => {
        pasted.push(file.name);
        return { ok: true };
      },
    });
    assert.equal(img.ok, true);
    assert.deepEqual(pasted, ["x.png"]);
    assert.equal(isPasteableUploadImage("image/png"), true);
    assert.equal(classifyTabPaste("image/png", 12).kind, "image");
    assert.equal(tabPastePlan("image/png", Buffer.from([9])).kind, "image");
  });
