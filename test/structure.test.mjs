...
describe("standalone product", () => {
  ...
  it("gateway is the only entry", () => {
    ...
    assert.match(gw, /parkSeatTarget/);
    assert.match(gw, /parkUserSeatWindows/);
    assert.match(gw, /closeUnassignedSeatTabs/);
    assert.match(gw, /seatOpenFlags/);
    assert.match(gw, /claimedTargetIds/);
    const leaveBlock = gw.slice(gw.indexOf('url.pathname === "/api/presence/leave"'), gw.indexOf("const open = url.pathname.match"));
    assert.match(leaveBlock, /releaseUserSeats/);
    assert.doesNotMatch(leaveBlock, /parkUserSeatWindows/);
    const idleBlock = gw.slice(gw.indexOf("for (const seat of seats.idleTabs())"), gw.indexOf("}, 15_000)"));
    assert.doesNotMatch(idleBlock, /closeSeatTab/);
    assert.doesNotMatch(idleBlock, /seats\.release/);
    assert.match(gw, /disableDeskSplitScreen/);
    assert.match(gw, /function closeSeatTab/);
    assert.doesNotMatch(gw, /await withDeadline\(closeTarget/);
    assert.match(gw, /deskHasChatGPTSession/);
    assert.match(gw, /runEnableJob/);
    assert.doesNotMatch(gw, /waitForDebugger/);
    assert.doesNotMatch(gw, /waitForDesk\(id, 45_000\)/);
    assert.match(gw, /enableDeskSplitScreen/);
    assert.match(gw, /SPLIT_SCREEN_DISABLED/);
    assert.match(gw, /createMemberProject/);
    assert.match(gw, /ensureParkedMemberSeat/);
    assert.match(gw, /warmAllParkedMemberSeats/);
    const listenBlock = gw.slice(gw.indexOf("server.listen"));
    assert.doesNotMatch(listenBlock, /warmAllParkedMemberSeats/);
    ...
    assert.match(gw, /拆除账号容器超时/);
    assert.match(gw, /closeTarget refuses the last\/primary page/);
    const createFn = gw.slice(gw.indexOf("async function createMemberProject"), gw.indexOf("async function lockSeatToProject"));
    assert.match(createFn, /seats\.claim/);
    assert.doesNotMatch(createFn, /finally/);
    assert.match(gw, /lockMemberToStoredProject/);
    assert.match(gw, /CHATGPT_NOT_LOGGED_IN/);
    ...
    assert.doesNotMatch(gw, /ONBOARD_YIELD_MS/);
    assert.doesNotMatch(gw, /function kickOnboard/);
    ...
    assert.doesNotMatch(gw, /kickOnboard/);
    ...
    assert.match(readFileSync(resolve(root, "lib/users.mjs"), "utf8"), /readyOn/);
    ...
    assert.match(ui, /已移除/);
    assert.match(ui, /未能移除/);
    const html = readFileSync(resolve(root, "gateway/web/index.html"), "utf8");
    ...
    assert.match(ui, /多人分屏暂未开放/);
    assert.doesNotMatch(ui, /每个账号单独开。开了才能几个人同时用这个号/);
    ...
    const cdpPanel = settingsBlock.slice(settingsBlock.indexOf("<b>多人分屏</b>"), settingsBlock.indexOf("<b>复制粘贴</b>"));
    assert.match(cdpPanel, /多人分屏暂未开放/);
    assert.doesNotMatch(cdpPanel, /data-cdp-toggle/);
    assert.doesNotMatch(cdpPanel, /data-cdp-master|cdp-master/);
    assert.doesNotMatch(cdpPanel, /assist-note/);
    assert.doesNotMatch(cdpPanel, /每个账号单独开/);
    ...
    const chromeBar = css.slice(css.indexOf(".chrome {"), css.indexOf(".back {"));
    assert.match(chromeBar, /display: flex;/);
    assert.match(chromeBar, /height: 44px/);
    assert.match(chromeBar, /padding-top: 7px/);
    assert.match(chromeBar, /padding-bottom: 3px/);
    assert.match(css, /\.chrome-mid[\s\S]*transform: translateY\(3px\)/);
    assert.doesNotMatch(chromeBar, /grid-template-columns: 1fr auto 1fr/);
    assert.match(css, /\.frame-wait-copy/);
    assert.match(ui, /正在进入/);
    assert.match(ui, /waitForFrame/);
    assert.match(ui, /state\.entering/);
    assert.match(ui, /state\.reused/);
    ...
    assert.match(readme, /exclusive VNC/i);
    assert.match(readme, /多人分屏暂未开放/);
    ...
    assert.match(zh, /暂未开放/);
    assert.match(zh, /独占 VNC|独占 KasmVNC/);
    ...
    assert.match(readFileSync(resolve(root, "lib/screencast.mjs"), "utf8"), /armFileChooser/);
    assert.match(gw, /handleDeskFileUpload/);
    assert.match(gw, /applyDeskUpload/);
    assert.match(gw, /watchAdminFileChooser/);
    ...
    assert.match(chrome, /无法将项目设为仅项目记忆/);
    assert.match(chrome, /已有同名项目使用账号默认记忆/);
    assert.match(chrome, /setProjectOnlyOnForm/);
    assert.match(chrome, /confirmExistingMemory/);
    ...
    assert.match(gw, /seatStartUrl\(\{ cdp: cdp && sess\.user\.role !== "admin", projectUrl \}\)/);
    ...
    assert.match(wm, /F12/);
    assert.match(gw, /createParkedChatGPTTab/);
    assert.match(readme, /withdrawn/i);
    assert.match(readme, /409 CDP_OFF/);
    assert.doesNotMatch(readme, /completely impossible/i);
    assert.match(zh, /已撤回|暂未开放/);
