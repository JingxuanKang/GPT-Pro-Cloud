...
describe("standalone product", () => {
  ...
  it("gateway is the only entry", () => {
    ...
    assert.match(leaveBlock, /parkUserSeatWindows/);
    assert.doesNotMatch(leaveBlock, /releaseUserSeats/);
    ...
    assert.match(gw, /await closeSeatTab\(seat\)/);
    ...
    assert.doesNotMatch(gw, /SPLIT_SCREEN_DISABLED|多人分屏暂未开放/);
    ...
    assert.doesNotMatch(ui, /多人分屏暂未开放/);
    ...
    assert.match(ui, /cdp-empty/);
    assert.match(ui, /每个账号单独开。开了才能几个人同时用这个号/);
    assert.match(ui, /复制粘贴/);
    assert.match(ui, /到管理添加/);
    ...
    assert.match(readme, /members get (their own )?tab|tab seats/i);
    assert.match(readme, /admin.*(?:full|complete|uncut) desktop/i);
    ...
    assert.match(zh, /第一人看整桌|管理员看完整桌面/);
    ...
    assert.match(deskCdpOnFn, /data\.deskCdp/);
