...
记忆隔离解决"共用账号但不共用上下文"。开启**多人分屏**是一项任务：已分配的每位成员都会得到一个以其用户名命名、设为**仅项目内记忆**的 ChatGPT 项目（在后台停放标签里创建，不展示给成员）。分屏已开时再勾选成员，会立刻创建该项目。成员打开账号时不会创建项目——直接落到已保存的 `https://chatgpt.com/g/g-p-<id>-<slug>/project`。项目内的对话不读写账号的全局记忆，成员之间互不泄漏上下文，每人的对话也归拢在各自的项目里。管理员没有 ChatGPT 项目，使用完整未裁切的桌面。

开启多人 / CDP 后，页面会把每位成员留在自己的项目里：加了 jail 的席位会藏掉整列左侧栏（别人的项目是没有 `/g/g-p-` href 的 DIV），点到或跳到别人的 `/g/g-p-…` 会回到自己的项目页。占用者不能再开 Chrome 标签、窗口、分屏或地址栏（Ctrl/Cmd+T/N/L/W/Tab、Shift+T、Alt+D、F6、F12、Ctrl+Shift+I/J/C、Ctrl+U，以及页面 `window.open`）。网关自己的分屏席位还在——每人一个隔离的 ChatGPT target。复制粘贴（Ctrl/Cmd+C/V）照常。这是页面体验锁定（CDP 注入 + `Page.navigate`），不是服务端权限——Cookie 仍是同一份。管理员完整桌面不裁切。
...
| `TAB_SEATS_MAX` | 开启多人后，每个账号同时打开的 chatgpt.com 分屏标签上限（默认 `3`，范围 1–8）。成员是独立标签；管理员看完整桌面。离开时停放窗口，下次进入复用同一 target |
...
一个 ChatGPT 账号仍然是一台桌面容器、一份 Chromium profile（`--user-data-dir=/config/chromium`）。关闭多人时两个人不该共用一只 VNC 鼠标。分屏席位要管理员在该账号上打开「允许多人同时使用」；未开启时第二人会被拒绝。

- 多人（CDP）**关闭**时，第一人看整桌（独占 KasmVNC），第二人会被拒绝（`409 CDP_OFF`）。
- 开启多人后成员每人都是独立标签，管理员看完整桌面（VNC，无裁切、无项目锁定），可以看到别人的 Chrome 标签。后来的成员在**同一只** Chromium 里新开 chatgpt.com 窗口（`newWindow: true`），并停到屏幕外，不会把主窗口改成标签。网关只推这一窗的像素（CDP `Page.startScreencast`），指针和键盘走 CDP `Input`。成员看不到标签栏，也看不到别人的 target。
- 卡片上的「断开」按人生效：只关掉那个人的标签（或 VNC），不杀别人的标签，也不停容器。
- 上限：`TAB_SEATS_MAX`（默认 3 个分屏；开启多人后每人都是标签）。离开时停放该成员窗口并保留 `targetId`，下次进入复用已有 target，不再 `createTarget`。45 秒空闲计时只清占用，不关窗口。关掉多人分屏、踢人、或取消分配时才会关掉停放的席位窗口。
- 成员席位会藏掉整列左侧栏，只看自己的项目工作区。跳到另一个 `/g/g-p-…/project` 会被弹回。若已保存的项目 URL 缺失，成员会收到「工作区未就绪」——由管理员再开一次分屏，或取消勾选再勾选该成员。分享和页面协助只在该账号开启多人分屏时可用。
- 去掉了 `--kiosk`。开启分屏时 Chromium 用普通窗口（不用 `--app`），额外席位窗口不会把进程带下去。多出来的目标是新窗口并停在屏幕外；成员看到的是页面视口，不是浏览器外壳。
