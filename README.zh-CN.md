...
多人同时共用一个 ChatGPT 登录**暂未开放**。一份 Plus Cookie 做不到按人隔离。每个账号同时只允许一人进入，走独占 VNC；第二人会被拒绝。设置页写着**多人分屏暂未开放**。
...
| 断开占用 | 在占用中的账号卡片上点「断开」，只撤掉**对方**的独占 VNC，容器还在。对方需重新登录。账号还在 |
...
分享对话走独占桌面里 ChatGPT 自带的 Share，复制后链接经剪贴板链路落到本机。

**多人分屏已撤回。** 一个 ChatGPT 登录就是一份 Cookie。仅项目记忆和藏侧栏只是表皮，不是权限。成员窗和管理员 VNC 挤在同一只 Chromium、同一把调试器和同一个窗口管理器上——会互相卡住、白屏、删除挂死。拷贝 profile 会撞 SingletonLock 或互踢会话。所以这一版不提供同一账号上的并发成员。

关闭 CDP 时（现在永远关闭）不做按人锁定（整桌一个 VNC）。若管理员把某个项目 URL 设成桌面起始页，kiosk 可以打开到那里，但那是整桌共用的一个项目，不是按成员隔离。
...
| `TAB_SEATS_MAX` | 多人分屏撤回后未使用（原先是并发分屏上限，默认 `3`） |
...
**多人分屏 / 分屏席位暂未开放。** 一个 ChatGPT 账号仍然是一台桌面容器、一份 Chromium profile（`--user-data-dir=/config/chromium`）。两个人不该共用一只 VNC 鼠标。

- 第一人看整桌（独占 KasmVNC），第二人会被拒绝（`409 CDP_OFF`）。
- 设置页写着**多人分屏暂未开放**。没有可开关、没有停放的成员窗、也不跑 CDP 开启任务。
- 卡片上的「断开」只撤掉当前占用者的独占 VNC，不停容器。
- 管理员和成员走同一条独占桌面路径。删除额外账号 / 移除成员不等调试器。
