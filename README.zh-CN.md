...
- 开启多人后成员每人都是独立标签，管理员看完整桌面（VNC，无裁切、无项目锁定），可以看到别人的 Chrome 标签。后来的成员在**同一只** Chromium 里后台新开 chatgpt.com 标签（`newWindow: false`），并停到屏幕外。网关只推这一页的像素（CDP `Page.startScreencast`），指针和键盘走 CDP `Input`。成员看不到标签栏，也看不到别人的 target。
